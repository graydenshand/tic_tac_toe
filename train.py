...
with open("tic_tac_toe/player1_states.json", "w") as f:
  sorted_states = {k:v for k,v in sorted(player1.state_scores.items(),key=lambda x:x[1], reverse=True)}
  f.write(json.dumps(sorted_states))

with open("tic_tac_toe/player2_states.json", "w") as f:
  sorted_states = {k:v for k,v in sorted(player2.state_scores.items(),key=lambda x:x[1], reverse=True)}
  f.write(json.dumps(sorted_states))